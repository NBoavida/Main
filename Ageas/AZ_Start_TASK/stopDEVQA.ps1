...
.NOTES
	...
	===========================================================================
	.DESCRIPTION
		Schedule task to Stop VM's Fom dev e  Qual 
...
#>
 ...
 Get-AzureVM  | Where-Object { `            $_.Name -like "SDAZSQL01" `         -or $_.Name -like "wdazsp2013-01" `
     ...
     Stop-azurevm  -ServiceName $_.Servicename -Name $_.Name  
...
$cmd.connection = $conn
$cmd.commandtext = "INSERT INTO ScheduleTask (Servicename,Action,source,date) VALUES('$_.Servicename','Stop VM', 'SDAZOP02', '$time' )"
$cmd.executenonquery()
$conn.close()
